...
if (isset($_POST['save'])) {
	...
	if ($recnum>0) {
		...
		$request="insert into `BU_kunden` (`".join("`,`",$f)."`) values ('".join("','",$values)."')";
	}
		
	// echo $request;
	$result = $db->query($request);
	if (!$result) {
		echo "Kunde konnte nicht bearbeitet/geändert werden<br>";
	}
...
if (isset($_POST['find_name'])) {
	...
	$request="select * from `BU_kunden` where `vorname` like '%".$vorname."%' and `nachname` like '%".$nachname."%'" ;
	$result = $db->query($request);
	$row = $result->fetch_assoc();
	if ($row) {
		$found=true;
...
<body><center>
<h1>Buchhaltung</h1>
<h2>Kunden anlegen/bearbeiten</h2>

<?php
// 	include "menu.php";
?>
<form action="kundeBearbeiten.php" method="POST";>
