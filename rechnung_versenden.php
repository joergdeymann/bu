...
// echo "vor Class Rechnung:";
include_once 'class/class_rechnung.php'; // Enthält class_io.php mit erstellung der $io=new IO();
// echo "nach class Rechnung";

if (!empty($_POST['send_mail'])) {
	include_once 'class/class_pdf.php';
	include_once 'class/class_phpmailer.php';
	// include 'class/class_io.php';
	// Wenns nicht geladen wird, dann wird SplitURL nicht geladen
	// Wenn es geladen wird, ist es schon definiert
}
// echo "vor Menu";
...
// print_r($cc);
// exit;
$m->setTo($send_to);  // Anders nennen in der eingabe: Rechnungsmail
// $m->setTo("test-k5ascmyuf@srv1.mail-tester.com"); // ############# Zum Testen
...
// $unsubscribe_http=""; if (!empty($rechnung->row_firma['web'])) $unsubscribe_http=$rechnung->row_firma['web'];
$io=new IO(); // Muss hier neu definiert werden, sonst wird die Klasse nicht erkannt, obschon die schon in class_rechnung bereits geladen und definiert worden ist.
// $url=$io->toDate("10.10.2024");
$url=$io->splitURL();
$unsubscribe_http=$url['protocol']."://".$url['dirname']."/re_unsubscribe.php"; 
$unsubscribe_mail=$rechnung->row_firma['rmail'];
$m->setUnsubscribe($unsubscribe_mail,$unsubscribe_http);
