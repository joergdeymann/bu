...
function XsetFaellig() {
	// Falls Rechnungsnummer da ist nicht neu berechnen, wurde dann schon geladen
	if (!empty($_POST['renr'])) {
		return; 
	}
	global $db;
...
function pdf() {
	...
	*/
	...
	function printPDF() {
		...
		var file="rechnung_printed.php?typ=1&renr="+renr+"&kdnr="+kdnr;		
...
if (isset($_POST['change']) && $save) {
	...
	// UPDATE Mahnung
	// $request="update `bu_mahn` set `datum`='".$_POST['datum']."' where firmanr='".$_SESSION['firmanr']."' and renr='".$_POST['renr']."' and mahnstufe=0";
	// echo $request."<br>";
	// $result = $db->query($request) or die(mysql_fehler()); 

	
	// UPDATE Rechnung
	$request="update `bu_re` set $r where firmanr='".$_SESSION['firmanr']."' and renr='".$_POST['renr']."' and typ='1'";
	// echo $request."<br>";exit;
	$result = $db->query($request) or die(mysql_fehler()); 
	if ($result) {
		$msg="Angebotssdaten wurde geändert.<br>";
	}
}
...
if (isset($_POST['save']) && $save) {
	...
	/*
		...
		Angebotsnummer erhöht sich wie Rechnungsnummer 
...
*/	
	...
	if ($result) {
		$msg="Angebot wurde neu angelegt. Jetzt nur noch die Posten hinzufügen<br>";
	}
}
...
if (isset($_POST['renr']) && !empty($_POST['renr'])) {
	$request='select * from bu_re where renr="'.$_POST['renr'].'" and firmanr="'.$_SESSION['firmanr'].'" and typ="1"';
	$result = $db->query($request) or die(mysql_fehler());
	/*
...
if (empty($_POST['renr']) ) {
	$renr="";
	$re_text='<b id="rand">Neues Angebot</b>';
} else {
	$renr=$_POST['renr'];
...
showHeader("Angebot erstellen/ändern",1);
// pdf();
// top: 250px;
echo '
...
echo '<table>';
echo '<tr><th>Angebotsnummer</th><td><input type="hidden" name="renr" size="15" value="'.$renr.'">';
echo $re_text; // Rechnungsnummer oder "Neue Rechnung"
echo '<input type="submit" name="find" value="Suchen" formaction="angebot_suchen.php"></td></tr>';
echo '<tr><th>Angebotsdatum</th><td><input type="date"  name="datum" value="'.$_POST['datum'].'"></td></tr>';
...
if ($dbfirma->get("re_input_individuell")) {
	...
	echo '<tr><th>Angebotsformular/Layout</th><td> <input type="text"  name="layout"         size="5"   value="'.$layout.'"></td></tr>';
}
...
*/
	...
	*/
	...
	echo '<input style="font-size:1.5em;" type="submit" name="findartikel" value="Artikel hinzufügen">';
	echo '<br><br>';
	echo '</form>';		
...
*/
	...
	echo '<button type="submit" name="add_re" formaction="rechnung_anlegen.php" formmethod="POST">als<br>Rechnung<br>anlegen</button>';
...
if (isset($_POST['renr'])) {
	$file="rechnung_print.php?renr=".$_POST['renr']."&mahnstufe=0&typ=1&firmanr=".$_SESSION['firmanr'];	
	echo "<script>";
	echo "setPDF('$file');";
	echo "</script>";
}
