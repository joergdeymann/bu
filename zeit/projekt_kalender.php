...
// echo "Kunde-Name:". $_POST['kunde_name'];

$mi = new mitarbeiter($db);
$mi->loadByRecnum($_SESSION['usernr']);
...
*/
$html_kunden ='<style>.test,datalist,option[value],list {color:green;!important}</style>';

$html_kunden.='<datalist id="kunden">';
$row=array();
$row['auftraggeber']=$mi->row['firmanr'];
$query="
	select * from bu_kunden
	where auftraggeber=".$mi->row['firmanr']." 
	order by firma";

$kunde->query($query);
while ($row=$kunde->next()) {		
	$html_kunden.='<option class="test" value="'.$row['recnum'].'">'.$row['firma'].'</option>';
}
...
$html.='<input type="submit" value="Auswahl" method="POST" action="projekt_kunde.php">';
$html.='</td></tr></table>';
$html.=$html_kunden;
...
if (!empty($_POST['anreise']) or !empty($_POST['abreise'])) {
	...
	<input '.$action.' name="fahrtkosten_nein" value="Nein" type="submit" style="'.$color2.'"></td></tr>';
}
if ($_POST['fahrtkosten_erstattung'] != 1) {
	...
	$html.='<tr><td>Strecke:        </td><td><input name="strecke"     value="'.$_POST['strecke']    .'" type="number" placeholder="0"  style="text-align:right;">km</td></tr>';
	$html.='<tr><td>Fahrten:        </td><td><input name="fahrten"     value="'.$_POST['fahrten']    .'" type="number" placeholder="0"  style="text-align:right;"></td></tr>';
}
$html.='</table>';
if (!empty($_POST['anreise']) or !empty($_POST['abreise'])) {
	$html.='</fieldset>';
...
foreach($_POST['equipment_name'] as $k =>$v) {
	...
	$html.='</td><td style="width:"1px;margin-left:10px;";>';
	$html.='<input type="submit" value="Auswahl" method="POST" action="./projekt_equipment.php">';
	
	$html.='</td></tr></table>';
	// $html.='<br style="margin-bottom:1em;">';

}
$html.='<div style="margin-right: 1px;text-align:right;"><input '.$action.' name="add_equipment" style="width:60px;" type="submit" value="+"></span></div>';
...
$html.='<table>';
$html.='<tr><td>Unterkunft nötig? </td><td><input '.$action.' name="unterkunft_ja" value="Ja" type="submit" style="'.$color1.'"><input '.$action.' name="unterkunft_nein" value="Nein" type="submit" style="'.$color2.'"></td></tr>';
...
if ($_POST['unterkunft_noetig']==1) {
	...
	$html.='<tr><td>vom Kunden gestellt ? </td><td><input '.$action.' '.$autofocus.' name="unterkunft_gestellt_ja" value="Ja" type="submit" style="'.$color1.'"><input '.$action.' name="unterkunft_gestellt_nein" value="Nein" type="submit" style="'.$color2.'"></td></tr>';
...
if ($_POST['unterkunft_gestellt'] == 1) {
	...
	$html.='</td><td style="width:"1px;margin-left:10px;";>';
	$html.='<input type="submit" value="Auswahl" method="POST" action="projekt_unterkunft.php">';
	$html.='</td></tr></table>';
	// $html.='<br style="margin-bottom:1em;">';
...
$html.='<textarea '.$autofocus.' name="info" style="width:calc(100vw - 37px);max-width:calc(100vw - 37px) !important;height:8em;">'.$_POST['info'].'</textarea>';
$html.='</fieldset>';
