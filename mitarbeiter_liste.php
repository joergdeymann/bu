...
if (count($_POST) > 0 and !empty($_SERVER['HTTP_REFERER'])) {
	if (basename($_SERVER['HTTP_REFERER']) == "projekt.php")  {
		echo "SESSION un mitrarbeiter_liste";
		$_SESSION['projekt']=$_POST;
		$_SESSION['projekt']['find_recnum']=true;
	}
}
...
function showMitarbeiter() {
	...
	if (!empty($_SESSION['projekt']['projekt_recnum'])) {
		$_POST['projekt_recnum']=$_SESSION['projekt']['projekt_recnum'];
	}
	if (!empty($_SESSION['projekt']['aufgabe_recnum'])) {
		$_POST['aufgabe_recnum']=$_SESSION['projekt']['aufgabe_recnum'];
	}
	while($row = $result->fetch_assoc()) {		
		$action="";
		$action ='<form style="display:inline;margin:0;padding:0;">';
		if (!empty($_POST['aufgabe_recnum'])) {
			$action.='<input type = "submit" value="zum Projekt hinzufügen" name="find_recnum" formmethod="POST" formaction="projekt_arbeiter.php">';
			$action.='<input type = "hidden" value="'.$_POST['aufgabe_recnum'].'" name="aufgabe_recnum">';
			$action.='<input type = "hidden" value="'.$_POST['projekt_recnum'].'" name="projekt_recnum">';
			$action.='<input type = "hidden" value="'.$row['recnum'].'"           name="mitarbeiter_recnum" >';
		} else {
			$action.='<input type = "hidden" name="recnum" value="'.$row['recnum'].'">';
			$action.='<input type = "submit" value="bearbeiten" name="find_recnum" formmethod="POST" formaction="mitarbeiter.php">';
