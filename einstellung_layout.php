...
$nr_standart=$r['rechnungs_layout'];
if ($r['rechnungs_layout'] == $_POST['nr']) $is_standart=true; 	
	
//Standartlayout neu setzten
if (isset($_POST['nr']) and isset($_POST['btn_standart'])) {
	$request="UPDATE bu_firma set rechnungs_layout='".$_POST['nr']."' where recnum='".$_SESSION['firmanr']."'";
	$result=$db->query($request);
}
...
&nbsp;
<?php
echo "<i>Vorlage <b>$nr_standart</b> wird als Standart verwendet.</i>";
?>

</td></tr>
<tr><th>Mahnstufe</th><td>             <input type="number" name="mahnstufe"       size="10" value="<?php echo $_POST['mahnstufe']; ?>"></td></tr>
<tr><th>Kunde (optional)</th><td>      
<input type="hidden" name="kdnr"       size="50" value="<?php echo $_POST['kdnr']; ?>">
<input type="hidden" name="kunde"       size="50" value="<?php echo $_POST['kunde']; ?>">
<?php if (!empty($_POST['kunde'])) echo $_POST['kunde']." "; ?>
<input type="submit" value="Liste" name="findkunde" formmethod="POST" formaction="kunde_suchen.php">
<?php 
if (!empty($_POST['kunde'])) {
	echo '&nbsp;<input type="submit" value="Reset" name="clear">';
} 
?>
</td></tr>
<tr><td colspan=2>
...
<input type="submit" name="btn_standart" value="Vorlage als Standart verwenden">
</td></tr></table>
...
<tr><th>Signatur<br>(HTML-Format)</th>			<td>  <textarea name="mail_signature" rows="6" style="width:calc(100% - 2px);padding:0;margin:0;"><?php echo $_POST['mail_signature'] ?></textarea></td></tr>
