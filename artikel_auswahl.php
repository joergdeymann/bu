...
if ((basename($_SERVER["HTTP_REFERER"]) == "angebot.php") and (count($_POST)>0)) {
	$typ=1;
} else {
	$typ=0;
}
...
<?php
	...
	while($row = $result->fetch_assoc()) {		
		...
		if ($typ==0) {
			...
			$action.='<input type = "submit" value="Wahl" name="return_artikelnr" formmethod="POST" formaction="angebot.php">';
		}
		$action.='<input type = "hidden" name="netto" value="'.$row['netto'].'">';
		$action.='<input type = "hidden" name="mwst" value="'.$row['mwst'].'">';
