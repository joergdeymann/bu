...
class DB_Posten {
	...
	public function start() {
		$request='select * from `bu_re_posten` where renr="'.$_POST['renr'].'" and firmanr="'.$_SESSION['firmanr'].'" order by `pos`';
		// echo $request."<br>";	
	...
	*/
	public function startExtend() {
		$request ='SELECT * FROM `bu_re_posten` LEFT JOIN `bu_artikel` ON bu_re_posten.artikelnr = bu_artikel.artikelnr WHERE renr="'.$_POST['renr'].'" and firmanr="'.$_SESSION['firmanr'].'" order by `pos`';
		// echo $request."<br>";	
		$this->result = $this->db->query($request) or die(mysql_fehler());
	}	
	...
	*/
	...
	private function getMaxPos(&$fields) {
		$request ='SELECT max(POS) as mx from bu_re_posten where renr="'.$fields['renr'].'" and firmanr="'.$_SESSION['firmanr'].'"';
