...
include "class/class_DB_Posten.php";
		...
		switch($dbfirma->get("re_input_leistung"))  {
			case 1:
				if (!empty($row['leistung'])) {
					...
					$_POST['leistungsmonat']=$dt->format("m");
					$_POST['leistungsjahr']=$dt->format("Y");
				}
				break;
...
if (isset($_POST['renr']) && !empty($_POST['renr'])) {
	$request='select * from bu_re where renr="'.$_POST['renr'].'" and firmanr="'.$_SESSION['firmanr'].'"';
	$result = $db->query($request) or die(mysql_fehler());
	/*
...
echo '<input type="submit" name="find" value="Suchen" formaction="rechnung_suchen.php"></td></tr>';
...
if (!empty($_POST['renr'])) {
	...
	*/
	...
	echo '<table border=0>';
	echo "<tr><th>Menge</th><th>Text</th><th>Betrag</th><th>Aktion</th></tr>";
	...
	while ($p=$posten->getNext()) {
		...
		$p['gesamt_mwst_betrag'] = sprintf("%.2f",(float)$p['gesamt_netto'] * (int)$p['mwst'] / 100);
...
*/
		...
		echo '<td style="min-width:70%;font-weight:1000;">'.$p['re_text'].'<br><textarea style="width:100%;height:10em;" name="beschreibung">'.$p['beschreibung'].'</textarea>';
		...
		echo '<input type="submit" name="posten_change" value="anpassen">';
		echo '<input type="submit" name="posten_delete" value="entfernen">';
		echo '</td>';	
		echo '</tr>';
		echo '</form>';
		echo '<tr><td colspan=4 style="height:2px;background-color:gray;margin:0;padding:0"></td></tr>';
	}
	echo '</table>';	
