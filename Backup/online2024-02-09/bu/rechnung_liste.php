...
if (isset($_POST['button_bezahlt'])) {
	$request="update `bu_re` set `bezahlt`=CURRENT_DATE where `firmanr` = ".$_SESSION['firmanr']." and `renr` = '".$_POST['renr']."'";
	// echo $request;
...
*/
	...
	*/
	...
	// var_dump ($_POST['what']);
	// exit;
	$where1="where bu_re.firmanr = ".$_SESSION['firmanr'];
	if ($_POST['bezahlt'] != "1") {
			...
			if ($row['layout'] == 0) { // Das muss bu_re Layout sein 0 = Standart Layout nehemn von firma
				if ($std_layout == 0) {					
					...
					if ($r) {
						...
						$std_layout=$row_layout['rechnungs_layout'];
					}
				}
				$row['layout']=$std_layout;
