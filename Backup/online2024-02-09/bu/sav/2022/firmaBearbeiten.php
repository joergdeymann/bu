...
function clearFields() {
	...
	if (isset($_POST['sichern'])) {
		...
		if ($_POST['recnum']>0) {
			...
			`hra`       ='".$post['hra']."', 
			`ustid`     ='".$post['ustid']."', 
			`betriebsnr`='".$post['betriebsnr']."', 
			`prio`      ='".$post['prio']."', 
			`logo`      ='".$post['logo']."' where recnum=".$post['recnum'];
		} else {
...
<?php
	if (isset($_POST['sichern'])) {
		if ($msg) {
			...
			echo "<h1>Die Firmendaten wurden erfolgreich geändert!<h1>";
			echo "<a href=\"index.php\">Weiter zum Menu</a>";
			exit;
		}
	}
?>
<form action="firmaBearbeiten.php" method="POST">
<input name="recnum" type="hidden" value="<?php echo $_POST['recnum'] ?>"> 
<table>
<tr><th colspan=2><b>Adressangaben</b></th></tr>
<tr><th>Firmenname</th><td><input type="text" name="firma" value="<?php echo $_POST['firma'] ?>" size=60> <input name="suchen" type="submit" value="Suchen"></td></tr>
