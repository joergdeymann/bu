	...
	*/
	...
	while($row = $result->fetch_assoc()) {	
		if ($row['name'] != $name) {
			...
			$mitarbeiter_recnum=$row['mitarbeiter_recnum'];
			$name=$row['name'];
		...
		if ($row['type'] == "AS") {	
			$dt1=new DateTime($row['time']);
			$dt2=new DateTime($_POST['datumbis']." 23:59:59");
			// $dt2=new DateTime($row['time']."+2d");

			// Lade 2 Tage mehr als in der Einngabe
			// für übernacht arbeiten , kann also
			// vorkommen das mehrere Tage als gewollt angezeigt werden
			if ($dt1 <= $dt2) {   	
				$az_s=$row['time'];				
				$as_r=$row['recnum'];
			} else {
		...
		if ($row['type'] == "AE") {	
			...
			$ae_r=$row['recnum'];
			// echo "AS:$az_s.<br>";
			if (($az_s >0) && ($az_e>0)) { // Hier wurde eine Zeit falsch gespeichert
				...
				$line.='<td>'.$worktime.'</td><td style="width: auto;">&nbsp;</td>';	
